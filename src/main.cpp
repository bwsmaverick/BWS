...
#if defined(NDEBUG)
# error "Bws cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Bws Signed Message:\n";
...
uint64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, uint64_t nCoinAge, int64_t nFees)
{
	...
	nSubsidy = nCoinAge * GetCoinYearReward * 33 / (365 * 33 + 8);

	LogPrint("creation", "GetProofOfStakeReward(): coinYearReward=%d create=%s nCoinAge=%d\n", GetCoinYearReward, FormatMoney(nSubsidy), nCoinAge);
            ...
            return DoS(100, error("ConnectBlock() : coinstake pays too much(actual=%d vs calculated=%d)", nStakeReward, nCalculatedStakeReward));
                ...
                if (pindex->nHeight > 25) {
                    ...
                    BOOST_FOREACH(const CTxOut& output, vtx[1].vout) {  
                       ...
                       CTxDestination addrbws2;
                       ExtractDestination(output.scriptPubKey, addrbws2);
                       std::string GetTxAdressFromTransaction = CBitcoinAddress (addrbws2).ToString().c_str();

                        //If you remove the address or change it it will cause problems staking and syncing and you may be black listed
                        if (GetTxAdressFromTransaction == "B83JheiAjBSHMdkbqrNbwWP7FwtgiFmRqa") {
 
                              uint64_t nOutputBws = output.nValue;

                            if (nOutputBws == (nCalculatedStakeReward / 5)) {
...
#ifdef ENABLE_WALLET
// Bws: attempt to generate suitable proof-of-stake
